    ...
    <div class="flex flex-col w-full justify-center pl-2 pt-0.5 pb-0.5 gap-1">
      ...
      <div class="flex flex-row text-xs text-gray-400 justify-between">
        <div class="flex-1 transition truncate mr-2">
          📷 {{ demo.snapshot_count || 0 }} screens
        </div>
      </div>

      <!-- Custom Links Section -->
      <div v-if="has_custom_links" class="mt-2">
        <div class="text-xs text-gray-500 mb-1">
          🔗 Custom Links ({{ active_custom_links_count }})
        </div>
        <div class="flex flex-wrap gap-1">
          <div
            v-for="(link, linkId) in active_custom_links"
            :key="linkId"
            class="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-200 cursor-pointer hover:bg-blue-100 transition"
            :title="link.description || link.title"
            @click.stop="open_custom_link(link.url)"
          >
            {{ link.title }}
          </div>
        </div>
...
const has_custom_links = computed(
  () =>
    props.demo.custom_links && Object.keys(props.demo.custom_links).length > 0
);

const active_custom_links = computed(() => {
  if (!props.demo.custom_links) return {};

  const active_links: any = {};
  Object.entries(props.demo.custom_links).forEach(
    ([link_id, link]: [string, any]) => {
      if (link.is_active) {
...
const active_custom_links_count = computed(
  () => Object.keys(active_custom_links.value).length
);

const open_custom_link = (url: string): void => {
  if (url) {
